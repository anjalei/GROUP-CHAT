...
document.getElementById("removeMemberBtn").addEventListener("click", removeMemberFromGroup);
...
document.getElementById("changeAdminBtn").addEventListener("click", changeAdmin);

async function changeAdmin() {
    const newAdminEmail = prompt("Enter the email of the member to make admin:");
...
async function deleteGroup() {
    ...
    try {
        const res = await axios.delete(`http://localhost:3000/group/delete/${groupId}`, {
            headers: { Authorization: `Bearer ${token}` }
        });

        if (res.status === 200) {
            alert("✅ " + res.data.msg);
            const groupLi = document.querySelector(`#groupList li[data-group-id="${groupId}"]`);
